...
def fetch_pubmed(keywords, max_results, email, api_key="", days_back=2):
    start, end = _today_and_lookback(days_back)
    date_filter = f'AND ("{start:%Y/%m/%d}"[Date - Publication] : "{end:%Y/%m/%d}"[Date - Publication])'
    query = "(" + " OR ".join(f'"{k}"[Title/Abstract]' for k in keywords) + ") " + date_filter
...
def fetch_arxiv(keywords, categories, max_results, days_back=2):
...
def _fetch_rxiv(server, keywords, max_results, days_back=2):
...
def fetch_biorxiv(keywords, max_results, days_back=2):
    return _fetch_rxiv("biorxiv", keywords, max_results, days_back)


def fetch_medrxiv(keywords, max_results, days_back=2):
