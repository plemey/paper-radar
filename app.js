// ---- Global error surfacing (no console access on iPad, so show errors in-page) ----
...
window.addEventListener("unhandledrejection", (e) => {
  ...
  if (el) {
    el.style.display = "block";
    el.textContent = "Unhandled promise rejection: " + (e.reason && e.reason.message ? e.reason.message : e.reason);
  }
});
...
async function loadFeed() {
  ...
  if (!cfg.configured) {
    emptyState.textContent = "Not configured yet — open Settings (⚙) and fill in owner/repo/token.";
    return;
  }

  const feedUrl = `${RAW_BASE()}/data/feed.json?_=${Date.now()}`;
  const ratingsUrl = `${RAW_BASE()}/data/ratings.json?_=${Date.now()}`;
  emptyState.textContent = `Fetching ${feedUrl} …`;

  let feedRes, ratingsRes;
  try {
    feedRes = await fetch(feedUrl);
  } catch (e) {
    emptyState.textContent = `Network error fetching feed.json: ${e.message}`;
    return;
  }
  emptyState.textContent = `feed.json responded: HTTP ${feedRes.status}`;
  if (!feedRes.ok) {
    emptyState.textContent = `feed.json fetch failed: HTTP ${feedRes.status} for ${feedUrl}. Check owner/repo/branch in Settings.`;
    return;
  }
  ...
  try {
    ...
    emptyState.textContent = `feed.json didn't parse as JSON: ${e.message}`;
    return;
  }

  try {
    ratingsRes = await fetch(ratingsUrl);
  ...
  emptyState.textContent = `Loaded ${feed.length} papers from feed.json.`;
  render();
}
